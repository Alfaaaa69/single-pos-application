                ...
                <tbody class="divide-y divide-slate-100 text-slate-700">
                    @forelse($users as $user)
                        <tr class="hover:bg-slate-50/40 transition-colors">
                            <td class="px-6 py-4.5 font-bold text-slate-800 text-base">
                                {{ $user->name }}
                            </td>
                            ...
                            <td class="px-6 py-4.5">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider {{ $user->role->name === 'admin' ? 'bg-indigo-50 text-indigo-700 border border-indigo-100/50' : 'bg-amber-50 text-amber-700 border border-amber-100/50' }}">
                                    {{ $user->role->display_name }}
                                </span>
                            </td>
                            <td class="px-6 py-4.5">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold {{ $user->is_active ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-red-50 text-red-700 border border-red-100' }}">
                                    <svg class="w-1.5 h-1.5 mr-1.5 fill-current {{ $user->is_active ? 'text-emerald-500' : 'text-red-500' }}" viewBox="0 0 8 8"><circle cx="4" cy="4" r="3"/></svg>
                                    {{ $user->is_active ? 'Aktif' : 'Nonaktif' }}
                                </span>
                            </td>
                            <td class="px-6 py-4.5 text-right space-x-3">
                                <button wire:click="openEditModal({{ $user->id }})" class="text-indigo-600 hover:text-indigo-850 font-bold hover:underline transition-colors">Edit</button>
                                @if($user->id !== auth()->id() && $user->is_active)
                                    <button wire:click="delete({{ $user->id }})" wire:confirm="Nonaktifkan pengguna ini?" class="text-red-650 hover:text-red-805 font-bold hover:underline transition-colors">Nonaktifkan</button>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-400 font-medium bg-slate-50/20">
                    ...
                    <div class="relative p-6 flex-auto space-y-4">
                        ...
                        <div>
                            ...
                            <input type="email" 
                                   ...
                                   class="block w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/25 focus:bg-white transition-all duration-200" 
                                   required>
                            @error('email') <span class="text-xs text-red-500 mt-1.5 font-semibold block">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="password" 
                                   ...
                                   class="block w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-sm text-slate-800 placeholder-slate-400 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/25 focus:bg-white transition-all duration-200" 
                                   {{ $isEditing ? '' : 'required' }}>
                            @error('password') <span class="text-xs text-red-500 mt-1.5 font-semibold block">{{ $message }}</span> @enderror
                        </div>

                        <!-- Role -->
                        <div>
                            <label class="block text-sm font-bold text-slate-700 mb-1.5">Hak Akses / Role</label>
                            ...
                            @error('roleId') <span class="text-xs text-red-500 mt-1.5 font-semibold block">{{ $message }}</span> @enderror
                        </div>
                        ...
                        @if($userId !== auth()->id())
                            <div class="flex items-center">
                                ...
                                <label for="isActive" class="ml-2.5 block text-sm font-bold text-slate-700 cursor-pointer">Akun Aktif</label>
                            </div>
                        @endif
                    </div>
